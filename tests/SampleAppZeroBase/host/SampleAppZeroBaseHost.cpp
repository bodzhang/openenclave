...
int main(int argc, const char* argv[])
{
    ...
    if (argc != 2)
    {
        fprintf(
            stderr,
            "Usage: SampleAppZeroBaseHost.exe <path to  packaged enc/dev dll>\n"
            "Example: SampleAppZeroBaseHost.exe "
            "SampleAppZeroBase.dev.pkg\\SampleAppZeroBase.dll\n");
        return 1;
    }

    const uint32_t flags = oe_get_create_flags();

    result = oe_create_SampleAppZeroBase_enclave(
    ...
    printf("=== passed all tests (SampleAppZeroBase)\n");
